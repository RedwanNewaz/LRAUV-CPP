...
def get_model():
    # design network
    model = Sequential()
    model.add(Dense(150, input_shape=(1, 4)))
    model.add(Dense(2))
    model.compile(loss='mae')
    return model

def train(dataset, name, version):
    if not os.path.isdir('output/04'):
        os.mkdir('output/04')
    weight_name = "output/04/{}_{:.0f}.h5".format(name, version)
    loss_name   = "output/04/{}_{:.0f}.csv".format(name, version)
    scaler_name = "output/04/{}_{:.0f}.pkl".format(name, version)
    os.system('clear')
